...
class _MyRateState extends State<MyRate> {
  ...
  _product_list() async {
    ...
    try {
      ...
      // hideLoader(context);
      log('@@ product 2 ${data['data']['data'].length}');
      ...
      if (data?['data']['status'] == 1) {
        ...
        if (product_view.length > 0) {
          ...
          if (product_view.length >= size) {
            ...
            log('@@@ new product ${data['data']['data']}');
            for (dynamic obj in data['data']['data']) {
              for (dynamic objRes in product_view) {
                if (obj['id'] == objRes['product_id']) {
                  if (uniqueId1.add(objRes['product_id'].toString())) {}
  ...
  _product_list_added() async {
    ...
    try {
      ...
      if (data?['data']['status'] == 1) {
        ...
        setState(() {
          product_view = arr;
          _txtinput.text = _txtinput.text.toString() == ''
              ? data['data']['delivery_charge'].toString()
              : int.parse(data['data']['delivery_charge'].toString()) >
                      int.parse(_txtinput.text.toString())
                  ? _txtinput.text.toString()
                  : data['data']['delivery_charge'].toString();
        });
  ...
  submit() async {
    ...
    for (var i = 0; i < duplicates.length; i++) {
      if (duplicates[i]['rate'] != 0 && duplicates[i]['rate'] != '') {
    ...
    if (arr.length == 0) {
      ...
      data1 = {
        "service_list": arr,
        "delivery_charge":
            _txtinput.text.toString() == 0 ? "" : _txtinput.text.toString(),
      };
      showLoader(context);
      log('${data1}');
      try {
