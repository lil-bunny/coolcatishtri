...
class LaundryServiceMyRate extends StatefulWidget {
  const LaundryServiceMyRate({super.key});

  @override
  State<LaundryServiceMyRate> createState() => _LaundryServiceMyRateState();
}

class _LaundryServiceMyRateState extends State<LaundryServiceMyRate> {
  ...
  _product_list() async {
    ...
    try {
      ...
      if (data?['data']['status'] == 1) {
        ...
        setState(() {
          product = newArray;
          product1 = resultArray.isEmpty ? arr1 : resultArray;
          // duplicates1 = [];
        });
      } else {
        log("error!");
  ...
  _product_list_added() async {
    ...
    try {
      ...
      data1 = "ls_id=" + routes['id'];
      final data = await ApiService().sendGetRequest(
          '/v1/delivery-boy/laundry-service/laundry-product-list-with-price?${data1}');
      if (kDebugMode) {
        print(data);
      }
      hideLoader(context);
      ...
      if (data?['data']['status'] == 1) {
        ...
        setState(() {
          product_view = arr;
          _txtinput.text = data['data']['delivery_charge'] == null
              ? '0'
              : data['data']['delivery_charge'].toString();
        });
    ...
    if (arr.length == 0) {
      ...
      try {
        ...
        final data = await ApiService().sendPostRequest(
            '/v1/delivery-boy/laundry-service/add-update-laundry-product-price',
            data1);
        if (kDebugMode) {
          log('@@ ${data}');
        }
