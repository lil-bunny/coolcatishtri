...
class _SpecialServiceRateState extends State<SpecialServiceRate> {
  ...
  Set<String> uniqueIdProduct = HashSet<String>();
  dynamic duplicates = [];
  dynamic duplicates1 = [];
  dynamic duplicatesProduct = [];
  ...
  ScrollController _childController = ScrollController();
  ...
  void initState() {
    ...
    _childController.addListener(() {
      if (_childController.position.extentAfter == 0) {
        _parentController.jumpTo(_parentController.position.maxScrollExtent);
      } else {
        if (_childController.position.pixels < 0) {
          _parentController.jumpTo(
            _parentController.offset + _childController.position.pixels,
          );
        }
      }
    });
      ...
      if (data?['data']['status'] == 1) {
        ...
        if (product_view.length > 0) {
          ...
          for (int? i = 0; i! < size!; i++) {
            ...
            arr1.add({
              "product_id": data['data']['data'][i]['id'],
              "rate": "0",
            });
          }
        }
        ...
        if (duplicates.length > 0) {
          for (int i = 0; i < newArray.length; i++) {
            for (int j = 0; j < duplicates.length; j++) {
              log('@@@ tes1 ${duplicates}');
              // log('>>>>>>>>>>>>>>>>>>>>>>>>>>>> ${newArray[i]['id'].toString() + ' ' + duplicates[j]['product_id'].toString()}');
              if (newArray[i]['id'] == duplicates[j]['product_id'] ||
                  newArray[i]['product_id'].toString() ==
                      duplicates[j]['product_id'].toString()) {
                // log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>${duplicates[j]['rate']}');
                // setState(() {
          ...
          //     // log('@@@ tes1 ${duplicates + ' ' + newArray}');
          //     // log('>>>>>>>>>>>>>>>>>>>>>>>>>>>> ${newArray[i]['product_id'].toString() + ' ' + duplicates[j]['product_id'].toString()}');
          //     if (newArray[i]['id'].toString() ==
          //             duplicates1[j]['product_id'].toString() ||
          //         newArray[i]['product_id'].toString() ==
          //                 duplicates[j]['product_id'].toString() &&
          //             duplicates1[j]['rate'] > 0) {
          //       // log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>${duplicates[j]['rate']}');
          //       // setState(() {
          //       duplicates[i]['rate'] = duplicates1[j]['rate'];
        ...
        setState(() {
          product = newArray;
          product1 = resultArray.length == 0 ? arr1 : resultArray;
          // duplicates1 = [];
        });
      } else {
        log("error!");
  ...
  _product_list_added() async {
    ...
    try {
      ...
      String data1 = '';
      data1 = 'customer_id=' + routes['customer_id'];
      final data = await ApiService().sendGetRequest(
  ...
  submit() async {
    ...
    if (arr.length == 0) {
      showErrorPopup(context, 'Alert!',
          "Please enter rates in each of the Service segments before submitting and enter amount that should be greater than 0");
    } else if (_txtinput.text == '') {
      showErrorPopup(context, "Alert!", 'Enter delivery charges');
    } else {
      data1 = {
        "service_list": arr,
        "delivery_charge": _txtinput.text.toString(),
